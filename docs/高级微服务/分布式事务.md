...
如果此时要保证**可用性**，就不能等待网络恢复，那node01、node02与node03之间就会出现数据**不一致。**
...
- 事务的强一致性，满足ACID原则。
- 常用数据库都支持，实现简单，并且没有代码侵入  基于关系形数据库实现的
...
- 注册分支事务
- **记录undo-log（数据快照）这里记录的是更新前和更新后的快照**
- 执行业务sql并提交
- 报告事务状态
...
我们在account-service项目中的`cn.beisheng.account.service`包中新建一个接口，声明TCC三个接口：

```java
package cn.beisheng.account.service;
...
在account-service服务中的`cn.beisheng.account.service.impl`包下新建一个类，实现TCC业务：

```java
package cn.beisheng.account.service.impl;

import cn.beisheng.account.entity.AccountFreeze;
import cn.beisheng.account.mapper.AccountFreezeMapper;
import cn.beisheng.account.mapper.AccountMapper;
import cn.beisheng.account.service.AccountTCCService;
