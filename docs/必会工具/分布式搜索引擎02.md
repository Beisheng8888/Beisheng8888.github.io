...
- range查询：根据数值范围查询，可以是数值、日期的范围   其中e是等于的意思
...
换句话来说，在地图上找一个点作为圆心。ho，以指定距离为半径，画一个圆，落在圆内的坐标都算符合条件：
...
因此，我们在`cn.beisheng.hotel.pojo`包下定义一个实体类：
...
因此，我们在`cn.beisheng.hotel.pojo`中定义返回结果：
...
因此，我们在`cn.beisheng.hotel.web`中定义HotelController：
...
1）在`cn.beisheng.hotel.service`中的`IHotelService`接口中定义一个方法：
...
2）实现搜索业务，肯定离不开RestHighLevelClient，我们需要把它注册到Spring中作为一个Bean。在`cn.beisheng.hotel`中的`HotelDemoApplication`中声明这个Bean：
...
3）在`cn.beisheng.hotel.service.impl`中的`HotelService`中实现search方法：
...
修改在`cn.beisheng.hotel.pojo`包下的实体类RequestParams：
...
在`cn.beisheng.hotel.service.impl`的`HotelService`的`search`方法中，添加一个排序功能：
...
给`cn.beisheng.hotel.pojo`包下的HotelDoc类添加isAD字段：
...
修改`cn.beisheng.hotel.service.impl`包下的`HotelService`类中的`buildBasicQuery`方法，添加算分函数查询：
