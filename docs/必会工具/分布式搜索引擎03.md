...
在`cn.beisheng.hotel.web`包的`HotelController`中添加一个方法，遵循下面的要求：
...
在`cn.beisheng.hotel.service.IHotelService`中定义新方法：
...
在`cn.beisheng.hotel.service.impl.HotelService`中实现该方法：
...
```java
package cn.beisheng.hotel.pojo;
...
1）在`cn.beisheng.hotel.web`包下的`HotelController`中添加新接口，接收新的请求：
...
2）在`cn.beisheng.hotel.service`包下的`IhotelService`中添加方法：
...
3）在`cn.beisheng.hotel.service.impl.HotelService`中实现该方法：
...
在hotel-admin和hotel-demo中的`cn.beisheng.hotel.constatnts`包下新建一个类`MqConstants`：

```java
package cn.beisheng.hotel.constatnts;
...
```java
package cn.beisheng.hotel.config;

import cn.beisheng.hotel.constants.MqConstants;
...
1）首先在hotel-demo的`cn.beisheng.hotel.service`包下的`IHotelService`中新增新增、删除业务
...
2）给hotel-demo中的`cn.beisheng.hotel.service.impl`包下的HotelService中实现业务：
...
在hotel-demo中的`cn.beisheng.hotel.mq`包新增一个类：

```java
package cn.beisheng.hotel.mq;

import cn.beisheng.hotel.constants.MqConstants;
import cn.beisheng.hotel.service.IHotelService;
