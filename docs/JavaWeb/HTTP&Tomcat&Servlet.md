...
在我们日常的生活中，经常会使用浏览器去访问`百度`、`京东`、`123`等这些网站，这些网站统称为Web网站。如下就是通过浏览器访问123官网的界面:

​	知道了什么是Web，那么JavaWeb又是什么呢？顾名思义==JavaWeb就是用Java技术来解决相关web互联网领域的技术栈。

​	等学习完JavaWeb之后，你就可以使用Java语言开发我们上述所说的网站。而国内很多大型网站公司也是首选Java语言来解决web互联网相关的问题。那都有哪些公司的系统是使用Java语言的呢?
...
* Web服务器软件有很多，我们中将学习的是目前最为常用的==Tomcat==服务器
...
>
> 		1. 缺少JDK。JAVA_HOME
> 		1. 缺少tomcat自身的home目录作为环境变量（在idea中不需要设置，自动配置）。CATALINA_HOME。该变量的值设置为tomcat的根目录即可。
> 		1. 端口冲突。tomcat默认端口号为8080。如果该端口被占用，则无法启动。netstat -ano | findstr 8080。找到占用了该端口的应用，强制关闭。还可以更换tomcat端口。
    ...
    <plugins>
        ...
        <plugin>
            ...
            <configuration>
				...
				-->
                <path>/</path>
                <!--解决get方式用url传参的时候的中文乱码问题，此问题仅在tomcat7及以下版本出现-->
...
* Servlet运行在Servlet容器(web服务器)中，其生命周期由容器来管理，分为4个阶段：
  ...
  3. ==请求处理==：==每次==请求Servlet时，Servlet容器都会调用Servlet的==service()==方法对请求进行处理。每当tomcat调用共service方法时，都会开启一个新的线程执行内部的所有逻辑。

     > service是多线程执行的，所以尽量不要在servlet的成员位置定义变量，然后在service中修改。
...
* 一个Servlet,可以配置多个urlPattern
  ...
  * ==urlPattern配置规则==
    ...
    * 目录匹配
      ...
      /aaa/*      路径只要以/aaa/开头即可。后面的*代表任意字符串
  ...
  * 任意匹配
    ...
    / 叫做缺省匹配，默认匹配。default。如果有一个请求，请求的路径没有任何一个url-pattern与之匹配，也就是没有一个Servlet接受该请求。那么该请求就会给缺省路径的Servlet处理。所以这样的Servlet在一个项目中最多只能有一个。但是在Tomcat中，自带一个Servlet配置为了/。
...
1. urlPattern总共有四种配置方式，分别是精确匹配、目录匹配、扩展名匹配、缺省匹配
2. 五种配置的优先级为 精确匹配 > 目录匹配> 扩展名匹配  > 缺省匹配
