...
看到了前端编写的这三个技术效果后，学习的是HTML，学习HTML其实就是学习预定义的这些标签。
...
你在访问其他网站页面时会看到字体颜色是五颜六色的，我们可以该字体颜色吗？当然可以了
...
* HTML 语法松散
  ...
  比如 font 标签不加结束标签也是可以展示出效果的。但是建议你在写的时候还是不要这样做，严格按照要求去写。
...
* 创建模块
  ...
  在 Idea 中创建模块，而我们现在不需要写java代码，所以 `src` 目录就可以删除掉。在模块下创建一个html文件夹，所以的页面文件所部放在该文件夹下。模块目录如下
