...
![img](assets/image-20210831112115508.png)
...
![img](assets/image-20210831115645528.png)

通过浏览器打开该页面可以看到如下页面

![img](assets/image-20210831115902537.png)
...
通过插值表达式，可以获取数据模型中的数据，展示到页面上。本身不支持html语法解析。纯文本内容。

````java
{{数据模型中的key}}
````

v-text指令：相当于插值表达式。innerText

v-html指令：可以支持html语法解析。innerHTML



#### 1.3.1  v-bind & v-model 指令

![img](assets/image-20210831150101736.png)
...
>
> 单向绑定：只从数据模型中获取数据展示到视图。当数据改变时，视图会自动刷新；当视图改变时，数据不会发生变化。
>
> v-model通常给表单使用。
...
![img](assets/image-20210831150945931.png)

#### 1.3.2  v-on 指令

![img](assets/image-20210831151231955.png)
...
![img](assets/image-20210831151904081.png)
...
![img](assets/image-20210831154300325.png)
...
![img](assets/image-20210831155837801.png)

### 1.4  生命周期

生命周期的八个阶段：每触发一个生命周期事件，会自动执行一个生命周期方法，这些生命周期方法也被称为钩子方法。

![img](assets/image-20210831160239294.png)

下图是 Vue 官网提供的从创建 Vue 到效果 Vue 对象的整个过程及各个阶段对应的钩子函数

![img](assets/image-20210831160335496.png)

看到上面的图，大家无需过多的关注这张图。这些钩子方法我们只关注 `mounted` 就行了。

`created`：是对象创建完毕的生命周期函数（钩子），此时是最早可以设置数据的位置。
...
![img](assets/image-20210831161040800.png)

此案例只是使用 Vue 对前端代码进行优化，后端代码无需修改。

#### 1.5.2  查询所有功能

![img](assets/image-20210831161346678.png)
...
![img](assets/image-20210831163001830.png)

整体流程如下

![img](assets/image-20210831163035298.png)
...
![img](assets/image-20210831170943892.png)
...
1. 将资源 `04-资料\02-element` 下的 `element-ui` 文件夹直接拷贝到项目的 `webapp` 下。目录结构如下
   ...
   ![img](assets/image-20210831171856768.png)
...
4. 官网复制Element组件代码
   ...
   ![img](assets/image-20210831180730287.png)
...
![img](assets/image-20210831182349672.png)
...
![img](assets/image-20210831183433892.png)
...
![img](assets/image-20210831185223141.png)
...
![img](assets/image-20210831185612905.png)
...
![img](assets/image-20210831190328825.png)

将css样式拷贝到我们页面的 `head` 标签中，如下

![img](assets/image-20210831190419248.png)

将方法和模型数据拷贝到 Vue 对象指定的位置

![img](assets/image-20210831190534720.png)
...
1. **修改表头和数据**
   ...
   ![img](assets/image-20210831192032118.png)

   修改完页面后，还需要对绑定的模型数据进行修改，下图是对模型数据进行分析的图解

   ![img](assets/image-20210831192429806.png)

2. **给表格添加操作列**

   从之前的表格拷贝一列出来并对其进行修改。按钮是从官网的 `Button 按钮` 组件中拷贝并修改的

   ![img](assets/image-20210831192809304.png)
...
![img](assets/image-20210831214602954.png)
...
![img](assets/image-20210831214818516.png)

下面对官网提供的代码进行分析

![img](assets/image-20210831215609729.png)
...
![img](assets/image-20210831220034775.png)

点击 `显示代码` ，找到 `完整功能` 对应的代码，接下来对该代码进行分析

![img](assets/image-20210831220446390.png)
...
* `page-sizes` ： 每页显示个数选择器的选项设置。
  ...
  ![img](assets/image-20210831220820557.png)
