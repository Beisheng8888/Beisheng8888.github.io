...
* **对表单进行校验**
  ...
  ![img](assets/image-20210814174242688.png)
...
![img](assets/image-20210814182345236.png)
...
> 函数本身即是一个代码块，可以通过()调用；同时它还是一个对象，可以进行引用传递。用其他变量接收。
>
> ```js
> function add(a,b){
>     return a + b;
...
>
>
> let xxx = add;// 此处没有调用函数，仅仅是把函数对象的值传递给了xxx变量
> let result = xxx(1,2);
> alert(result);
> ```
>
> 使用arguments数组，可以在函数体内获取所有参数。
>
> js中函数没有重载，后定义会覆盖先定义的。
...
我们要操作浏览器的各个组成部分就可以通过操作 BOM 中的对象来实现。比如：我现在想将浏览器地址栏的地址改为 `https://www.beisheng.com` 就可以通过使用 BOM 中定义的 `Location` 对象的 `href` 属性，代码： `location.href = "https://beisheng.com";`
